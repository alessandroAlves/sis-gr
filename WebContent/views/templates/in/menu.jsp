<div style="width: 140px; padding: 8px; background-color:buttonface;">
  <ul class="nav nav-list">
	<li class="active"><a><i class="icon-th-large"></i> Menu</a></li>
	<li class="divider"></li>
	<li><a href="/sis-gr/agenda"><i class="icon-calendar"></i> Agenda</a></li>
	<li><a href="/sis-gr/listarReuniao"><i class="icon-comment"></i> Reuniões</a></li>
	<li><a href="/sis-gr/listarTarefas/${usuario.id}"><i class="icon-list"></i> Tarefas</a></li>
	<li class="divider"></li>
	<li><a href="/sis-gr/download"><i class="icon-download-alt"></i> Exportar</a></li>
  </ul>
</div>
